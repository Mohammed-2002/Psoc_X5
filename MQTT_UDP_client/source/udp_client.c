...
void udp_client_task(void *arg)
{
	...
	*/
    ...
    if (!wifi_connected) {
		printf("Wi-Fi connection not established. Cannot create UDP socket.\n");
		return;  // Exit the task or handle the error appropriately
	}
    ...
    while(true)
    {
        ...
        if(led_state_ack == LED_ON_CMD)
        {
            ...
            xQueueSend(motor_pins_queue, MOTOR_ON_STATE, portMAX_DELAY);
