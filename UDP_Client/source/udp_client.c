...
static cy_rslt_t send_data_to_server(const char *message);
...
cy_rslt_t send_data_to_server(const char *message)
{
    cy_rslt_t result;
    uint32_t bytes_sent = 0;
    cy_socket_sockaddr_t udp_server_addr = {
    ...
    result = cy_socket_sendto(client_handle, message, strlen(message), CY_SOCKET_FLAGS_NONE,
                                &udp_server_addr, sizeof(cy_socket_sockaddr_t), &bytes_sent);
    if(result == CY_RSLT_SUCCESS)
    {
